...
def df_create():
    if 'response1' in st.session_state and 'response2' in st.session_state and 'response3' in st.session_state and 'response4' in st.session_state:
        df = pd.concat([st.session_state.response1, st.session_state.response2, st.session_state.response3, st.session_state.response4], axis=1)
        return df
    return None
...
def main():
    ...
    if st.sidebar.button('Enviar cuestionario completo'):
        if 'response1' in st.session_state and 'response2' in st.session_state and 'response3' in st.session_state and 'response4' in st.session_state:
            ...
            st.warning(type(combined_response))
    ...
    if st.session_state.questionnaire_submitted:
        ...
        if visualizar_resultados == 'Sí':
            ...
            if df is not None:
                ...
                st.error('Error al crear el DataFrame con las respuestas.')
